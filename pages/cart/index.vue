<template>
      ...
      <section class="flex justify-between max-md:flex-col space-x-4">
        <div v-if="user" class="md:w-2/3 px-4">
          <div class="mt-16 md:mt-6">
            <button @click="logout" class="flex justify-end bg-orange-500 p-2 rounded-xl hover:bg-secondary-500">Logout</button>
        ...
        <div v-if="!user" class="flex justify-center items-center w-full h-full">
  ...
  const { products, removeProduct, addProduct, clearCart } = cartStore;
  ...
  onMounted(() => {
    if (cartStore.products.length > 0) {
      axios.post('/api/cart', { ids: cartStore.products }).then(res => {
  ...
  const productQuantity = (id: string) => {
    return cartStore.products.filter(productId => productId === id).length;
  };
  
  const total = computed(() => {
    return cartStore.products.reduce((sum, productId) => {
  ...
  //   try {
  //     const { $gAuth } = nuxtApp.vueApp.config.globalProperties;
